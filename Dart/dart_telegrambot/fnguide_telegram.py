...
def send_message(company, signal, case):
    try:
        strr = ("종목 :: " + company['dc:creator'] + " \n제목 :: " + company['title'].split('-')[1] + " \nlink :: " +
                company['link'] + "\nsignal :: " + signal + "\ncase :: " + str(case))
        push_notification(company, signal, case)
...
def new_algorithm(new_list):
    ...
    if len(new_list) > 0:
        for company in new_list:
            print(company['title'])
            no_need = ['기재정정', '코넥스', '증권발행', '첨부추가', '첨부정정', '연장결정']
            ...
            if not any(key in company['title'] for key in no_need):
                ...
                # 공개매수, 자기주식취득, 자진상장폐지
                    ...
                    if any(word in company['title'] for word in ['상장폐지']):
                        ...
                        if any(word in company['title'] for word in ['설명서', '폐지']):
                            break
                        else:
                            signal = 'HOLD'
                            case = "4, 공개매수"
...
def title_to_num(company_name):
    if any(word in company_name for word in ['주식분할결정']):
        ...
        return 10
    elif any(word in company_name for word in ['처분', '소각']):
        return 11
...
#### pusth test용
def push_notification(data, signal, case):
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    headers = {"Content-Type": "application/json", "Authorization": "key=" + config.push['api_key']}
    payload = {
        "notification": {
            "title": data['title'].split('-')[1],
            "body": "CASE ::: " + str(case) + "\nCOMPANY ::: " + data['dc:creator'] + "\nSIGNAL ::: " + signal,
            "sound": "default",
        ...
        "to": "/topics/" + config.push['topic'],
        "priority": "high",
        "restricted_package_name": ""
    }
    r = requests.post(config.push['url'], headers=headers, data=json.dumps(payload))
    print(r)
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            time.sleep(2)
            """테스트"""
            # new_list = [{'dc:creator':'녹십자','title':'녹십자-최대주주변경을수반하는주식양수도계약체결', 'link':'test'}]
            # new_algorithm(new_list)
            # time.sleep(2)
            continue
